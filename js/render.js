...
(function () {
  ...
  var makeEntity = function (imgName) {
    var tempObj = {
      ...
      likes: setLikes(render.LIKES_SPREAD_MAX, render.LIKES_SPREAD_MIN),
      comments: setCommentsArray(),
    };
    return tempObj;
  };
  var pushEntityToArray = function (entityArr) {
    for (var j = 1; j <= render.ENTITY_LIMIT; j++) {
  ...
  var setCommentsArray = function () {
    var tempArray = [];
    for (var a = 0; a <= window.utils.getRandomNum(render.COMMENTS_I_DESCR_SPREAD_MAX); a++) {
      var comentEntity = {
        avatar: 'img/avatar-' + window.utils.getRandomNum(render.AVATARS_COUNT, 1) + '.svg',
